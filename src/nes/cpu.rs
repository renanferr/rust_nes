...
impl CPU6502 {
    ...
    pub fn irq(&mut self) {
        ...
        if self.get_flag(Flags6502::I) == 0 {
            ...
            let addr: u16 = 0x0100;
            self.write(addr + (self.stkp as u16), ((self.pc >> 8) & 0x00FF) as u8);
            self.stkp -= 1;
            self.write(addr + (self.stkp as u16), (self.pc & 0x00FF) as u8);
            self.stkp -= 1;
            ...
            self.write(addr + (self.stkp as u16), self.status);
            self.stkp -= 1;
